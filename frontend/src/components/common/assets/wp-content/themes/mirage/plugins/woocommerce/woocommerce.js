/* global jQuery, MIRAGE_STORAGE */

jQuery( document ).on( 'action.ready_mirage', function() {
	...
	$document.on( 'action.resize_mirage', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.mirage_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		mirage_set_cookie( 'mirage_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	mirage_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function mirage_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						mirage_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function mirage_woocomerce_inc_dec_click(button) {
			...
			.on( 'click', function(e) {
				...
				jQuery.get(	link ).done(
					function( response ) {
						...
						mirage_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
						mirage_import_tags_link( response );
						...
						if ( posts_container.length > 0 ) {
							...
							MIRAGE_STORAGE['init_all_mediaelements'] = true;
							$document
	...
	$document.on( 'action.scroll_mirage', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < mirage_window_scroll_top() + mirage_window_height() * 1.5 ) {
	...
	//-------------------------------------------------------------------------
	if ( ! MIRAGE_STORAGE['select_container_disabled'] ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		mirage_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					mirage_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					mirage_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		mirage_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			mirage_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function mirage_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = mirage_window_width() + ( mirage_window_height() < $document.height() || mirage_window_scroll_top() > 0 ? 16 : 0 );
				}

				var tbl = jQuery( 'table.cart' );   // table_cart;

				if ( w < MIRAGE_STORAGE['mobile_layout_width'] ) {
				...
				if ( w >= MIRAGE_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
